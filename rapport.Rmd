...
# 7. MODÈLE PRÉDICTIF SIMPLE
# ===========================

# Régression linéaire simple avec les 3 variables principales
model_simple <- lm(Rating ~ Temp_Annuelle + Pluie_Annuelle + Soleil_Annuel, 
                   data = wines_simple)

# Prédictions
wines_simple$Rating_Predit <- predict(model_simple, wines_simple)

# Métriques du modèle
r2 <- summary(model_simple)$r.squared
mae <- mean(abs(wines_simple$Rating - wines_simple$Rating_Predit))
rmse <- sqrt(mean((wines_simple$Rating - wines_simple$Rating_Predit)^2))

print("=== MODÈLE PRÉDICTIF SIMPLE ===")
print(paste("R² (variance expliquée) :", round(r2, 4)))
print(paste("Erreur absolue moyenne (MAE) :", round(mae, 3)))
print(paste("Erreur quadratique (RMSE) :", round(rmse, 3)))
...
[À compléter avec vos données : "La température printanière montre un impact positif (r=0.23) tandis que les pluies printanières sont négativement corrélées (r=-0.15)"]

Été (Juin-Août) :

Période critique de maturation des raisins
Généralement la saison la plus influente sur la qualité finale
[À compléter : "L'ensoleillement estival présente l'impact le plus fort (r=0.52), confirmant l'importance de cette période pour la photosynthèse"]

Automne (Septembre-Novembre) :

Période de vendange et de finalisation de la maturation
Conditions sèches généralement favorables
[À compléter : "Les précipitations automnales montrent un impact négatif marqué (r=-0.38), reflétant les risques de dilution et de maladies"]
...
```{r, echo=TRUE, warning=FALSE, message=FALSE}
# 9. ANALYSE DES ANNÉES EXCEPTIONNELLES
# ======================================

# Évolution des notes par année (calcul conservé pour l'analyse)
yearly_stats <- wines_simple %>%
  group_by(Year) %>%
  summarise(
    Note_moyenne = mean(Rating),
    Temp_moyenne = mean(Temp_Annuelle),
    Pluie_moyenne = mean(Pluie_Annuelle),
    Soleil_moyen = mean(Soleil_Annuel),
    n_vins = n(),
    .groups = 'drop'
  )

# Identification des meilleures et pires années
best_years <- yearly_stats %>%
  arrange(desc(Note_moyenne)) %>%
  head(5)

worst_years <- yearly_stats %>%
  arrange(Note_moyenne) %>%
  head(5)

print("=== TOP 5 MEILLEURES ANNÉES ===")
print(best_years %>% select(Year, Note_moyenne, Temp_moyenne, Pluie_moyenne, Soleil_moyen))

print("=== TOP 5 PIRES ANNÉES ===")
print(worst_years %>% select(Year, Note_moyenne, Temp_moyenne, Pluie_moyenne, Soleil_moyen))

# 10. CONCLUSIONS SIMPLES
# ========================

print("=== CONCLUSIONS ===")

# Variable la plus corrélée
best_correlation <- names(rating_correlations)[which.max(abs(rating_correlations[-1]))]
best_corr_value <- max(abs(rating_correlations[-1]))

print(paste("1. Variable météo la plus influente :", best_correlation))
print(paste("   Corrélation :", round(best_corr_value, 3)))

if(best_corr_value > 0.3) {
  print("   → Impact FORT sur la qualité du vin")
} else if(best_corr_value > 0.15) {
  print("   → Impact MODÉRÉ sur la qualité du vin")
} else {
  print("   → Impact FAIBLE sur la qualité du vin")
}

print(paste("2. Capacité prédictive globale (R²) :", round(r2, 3)))
if(r2 > 0.25) {
  print("   → La météo explique une part SIGNIFICATIVE de la qualité")
} else if(r2 > 0.10) {
  print("   → La météo a un effet MODÉRÉ sur la qualité")
} else {
  print("   → La météo a un effet LIMITÉ sur la qualité")
}

print("3. Recommandations :")
if(rating_correlations["Temp_Annuelle"] > 0) {
  print("   → Les années plus chaudes tendent à produire de meilleurs vins")
} else {
  print("   → Les années plus fraîches tendent à produire de meilleurs vins")
}

if(rating_correlations["Pluie_Annuelle"] > 0) {
  print("   → Plus de précipitations = meilleure qualité")
} else {
  print("   → Moins de précipitations = meilleure qualité")
}

if(rating_correlations["Soleil_Annuel"] > 0) {
  print("   → Plus d'ensoleillement = meilleure qualité")
} else {
  print("   → Moins d'ensoleillement = meilleure qualité")
}
```
